...
// the pad is never selected; with zero views ViewT is int and the array is
// empty, so storing the (empty) roots is a no-op that never indexes it.
template <typename... ViewTs>
auto lg_view_array(const ViewTs&... views) {
  ...
  return Kokkos::Array<ViewT, sizeof...(ViewTs)>{views...};
}
