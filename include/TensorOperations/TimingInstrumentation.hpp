  ...
  void print_report() const {
    ...
    if (input_stage_load_count > 0) {
      std::printf("%-40s %15lld %15lld\n",
                  "Input Stage Load",
                  input_stage_load_count.load(),
                  input_stage_load_time.load());
    }
    ...
    if (store_write_count > 0) {
      std::printf("%-40s %15lld %15lld\n",
                  "Store Write",
                  store_write_count.load(),
                  store_write_time.load());
    }
...
// Timing instrumentation macros (disabled by default; enable with TENSOR_OPS_ENABLE_TIMING)
  ...
  #define TIMING_SCOPE_EXIT(counter_time, counter_count) \
    do { \
      ...
      counter_time.fetch_add(static_cast<long long>(_timer_end_ - _timer_start_), std::memory_order_relaxed); \
