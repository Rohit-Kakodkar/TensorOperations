...
struct NodeHandle<ReduceTag, ReduceFn, IntRank, Scalar, ExecSpace, ModesSeq,
  ...
  static constexpr int NumOps  = static_cast<int>(sizeof...(Ops));
  static constexpr int NumOut  = IntNumOut::value;
  using value_type             = Scalar;
  using exec_space             = ExecSpace;
...
constexpr int label_position() {
  constexpr auto m = seq_to_array(ModesSeq{});
  for (std::size_t i = 0; i < m.size(); ++i)
    if (m[i] == L) return static_cast<int>(i);
  return -1;
}
...
struct concat_label_seq<std::integer_sequence<int32_t, As...>,
                        std::integer_sequence<int32_t, Bs...>> {
  using type = std::integer_sequence<int32_t, As..., Bs...>;
};
template <typename A, typename B>
...
// Extent of label L read off the first operand that carries it, or -1.
template <int32_t L, typename Op>
int reduce_extent_or_neg(const Op& op) {
  constexpr int p = label_position<L, typename Op::modes_seq>();
  if constexpr (p >= 0)
    return op.shape()[static_cast<std::size_t>(p)];
  else
    return -1;
}
...
template <int32_t... Ls, typename... Ops>
Kokkos::Array<int, sizeof...(Ls)> reduce_shape_of(
    std::integer_sequence<int32_t, Ls...>, const Ops&... ops) {
