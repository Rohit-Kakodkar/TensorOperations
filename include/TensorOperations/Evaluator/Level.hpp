...
// A general contraction's output tile is the map's tile over its output
// labels, resolved by LevelGraph::add (an output label no operand carries has
// no other source of extent). `void` means the node was never added to a
// graph.
template <typename Node>
struct MemberOutTile<Node, GeneralContractionTag> {
  static_assert(
      !std::is_same_v<typename Node::tile_type, void>,
      "general contraction member: its tile is unresolved, which means this "
      "node was never handed to LevelGraph::add -- add() is what looks the "
